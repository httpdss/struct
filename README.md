...
- `introspect`: Generate a YAML structure file from an existing directory
...
## 🔍 Reverse Template Generation (Introspection)

STRUCT can generate a draft YAML structure file from an existing directory using the `introspect` command. This is useful for bootstrapping a STRUCT config from a real project layout.

**Usage:**

```sh
struct introspect --source ./my-folder --output structure.yaml
```

- `--source`: Path to the folder to scan.
- `--output`: Output YAML file (default: structure.yaml in the current directory).

**Ignoring files and folders:**

To skip files or folders during introspection, add patterns (one per line) to a `.structignore` file in the source directory. Example:
...
```yaml
# Auto-generated by `struct introspect` from source: ./my-folder
files:
  - README.md:
      type: file
      content: |
...
- Small files are embedded with `content:`.
- Large files use `content_from:` with a relative path.
- Folders are listed under `folders:`.
- Jinja variables (`{{`) are escaped to avoid accidental template rendering.
