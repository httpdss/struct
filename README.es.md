...
## 🔍 Generación Inversa de Plantillas (Introspección)

STRUCT puede generar un archivo YAML de estructura a partir de un directorio existente usando el comando `introspect`. Esto es útil para crear rápidamente una configuración STRUCT desde la estructura real de un proyecto.

**Uso:**

```sh
struct introspect --source ./mi-carpeta --output structure.yaml
```

- `--source`: Ruta de la carpeta a analizar.
- `--output`: Archivo YAML de salida (por defecto: structure.yaml en el directorio actual).

**Ignorar archivos y carpetas:**

Para omitir archivos o carpetas durante la introspección, agrega patrones (uno por línea) a un archivo `.structignore` en el directorio fuente. Ejemplo:
...
```yaml
# Auto-generated by `struct introspect` from source: ./mi-carpeta
files:
  - README.md:
      type: file
      content: |
  ...
  - src/main.py:
      type: file
      content_from: file://src/main.py
folders:
  - src:
      type: directory
```

- Los archivos pequeños se incluyen con `content:`.
- Los archivos grandes usan `content_from:` con la ruta relativa.
- Las carpetas se listan bajo `folders:`.
- Las variables Jinja (`{{`) se escapan para evitar renderizado accidental.
